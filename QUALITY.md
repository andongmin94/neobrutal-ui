...
The `verify` job covers source contracts, builds, installed-consumer integration, aggregate installs and clean generation. `verify-browsers` runs each existing Playwright project in its own job: desktop light, mobile dark, Chromium reflow, Firefox desktop and WebKit mobile. These jobs preserve the configurations, cases, assertions, retries and timeouts; they do not share one cumulative execution budget. `verify-deployment` requires all source, independent-installation and documentation jobs to succeed, then verifies the exact production commit, registry and representative installs. `verify-deployed-browsers` runs the same three cross-browser projects against that deployment.

The `docs-review-*` and `deployed-docs-review-*` artifacts retain per-project page results and scoped state images. Full traces and videos are retained for failures, not duplicated for every successful run. Installation artifacts contain item-level results and logs. Check `verify`, all `verify-installations` and `verify-browsers` jobs, `verify-deployment`, and all `verify-deployed-browsers` jobs on the final commit before reporting release checks complete.
