    ...
    <Card className="min-w-0" data-chart-recipe="capacity">
      <CardHeader>
        <p className="font-mono text-xs uppercase tracking-widest">
          Delivery / Compare against a plan
        </p>
        <CardTitle>
      ...
      <CardContent className="grid min-w-0 gap-6">
        ...
        <dl className="grid gap-4 border-y-2 border-border py-4 sm:grid-cols-3" aria-live="polite">
          {[
            ...
            ["Net variance", signed(delivered - planned)],
          ].map(([label, value]) => (
            <div key={label}>
        ...
        <figure
          aria-label={
            variance
              ? "Signed delivery variance by sprint"
              : "Planned and delivered items by sprint"
          }
          className="min-w-0"
        >
                ...
                content={
                  <ChartTooltipContent
                    formatter={(value, name) =>
                      `${config[name as keyof typeof config]?.label ?? name}: ${Number(value)} items`
                    }
                  />
                }
              />
          ...
          <figcaption className="mt-3 text-xs leading-5">
            {variance
              ? "Above zero: more items delivered than planned. Below zero: fewer."
              : "Outlined / dashed: planned. Filled / solid: delivered. Both start at zero."}
          </figcaption>
        </figure>
        <output className="border-l-4 border-main pl-3 text-sm leading-6">
          {metPlan} of {data.length} sprints met or exceeded plan. The variance is delivered minus
          planned, not a percentage.
        </output>
        <details className="min-w-0 border-t-2 border-border pt-4">
          ...
          <table className="mt-3 w-full text-left text-xs tabular-nums">
            <caption className="pb-3 text-left">
              {team}. Item counts for the same four sprints.
            </caption>
            <thead>
              <tr>
                {["Sprint", "Planned", "Delivered", "Variance"].map((label) => (
