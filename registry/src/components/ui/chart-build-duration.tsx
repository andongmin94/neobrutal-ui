...
const config = {
  ...
  warm: {
    label: "Cached build",
    color: "color-mix(in srgb, var(--chart-2) 50%, var(--foreground))",
  },
    ...
    <Card className="min-w-0" data-chart-recipe="builds">
      <CardHeader>
        <p className="font-mono text-xs uppercase tracking-widest">
          Builds / Make the comparison explicit
        </p>
        <CardTitle>
          <h3>Build duration</h3>
        </CardTitle>
        <CardDescription>
          Paired cold and cached runs in seconds. Illustrative measurements of the same builds.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid min-w-0 gap-6">
        ...
        <dl className="grid gap-4 border-y-2 border-border py-4 sm:grid-cols-3" aria-live="polite">
          {[
            ["Mean cold run", `${coldMean.toFixed(1)} s`],
            ["Mean cached run", `${warmMean.toFixed(1)} s`],
            ["Mean time saved", `${(coldMean - warmMean).toFixed(1)} s`],
          ].map(([label, value]) => (
            <div key={label}>
        ...
        <figure aria-label="Cold and cached build duration in seconds" className="min-w-0">
            ...
            >
              ...
              {series !== "cold" && (
                <Line
                  ...
                  strokeWidth={2}
                  strokeDasharray="8 4"
                  dot={{ r: 4, fill: "var(--secondary-background)", strokeWidth: 2 }}
                  isAnimationActive={false}
                />
              )}
            </LineChart>
          ...
          <figcaption className="mt-3 text-xs leading-5">
            Solid / filled dots: cold. Dashed / open dots: cached. Dotted horizontal line: 220 s
            cold-run budget.
          </figcaption>
        </figure>
        <output className="border-l-4 border-main pl-3 text-sm leading-6">
          {breaches} of {data.length} cold runs exceeded budget. Summaries and the table compare
          both runs even when a plotted series is hidden.
        </output>
        <details className="min-w-0 border-t-2 border-border pt-4">
          ...
          <table className="mt-3 w-full text-left text-xs tabular-nums">
            <caption className="pb-3 text-left">
              Paired run durations. Time saved is cold minus cached, in seconds.
            </caption>
            <thead>
              <tr>
                {["Build", "Cold (s)", "Cached (s)", "Saved (s)"].map((label) => (
