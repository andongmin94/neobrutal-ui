...
test("optional help, custom IDs, refs, events, and field errors remain independent", async ({ page }) => {
  ...
  expect(await descriptions(first)).toEqual([
    { id: await first.getAttribute("aria-describedby"), text: "This help belongs to the application." },
  ]);
  await expect(second).not.toHaveAttribute("aria-describedby");
  await preview.getByRole("button", { name: "Show help", exact: true }).click();
  await expect.poll(async () => (await descriptions(first)).map(({ text }) => text)).toEqual([
    "This help belongs to the application.", "Optional field help.",
  ]);
