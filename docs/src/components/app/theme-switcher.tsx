...
export function ThemeSwitcher() {
  ...
  const handleThemeChange = () => {
    ...
    setTimeout(() => {
      ...
      if (newTheme === "dark") {
        r.style.setProperty("--background", r.style.getPropertyValue("--dark-background"));
        r.style.setProperty("--main", r.style.getPropertyValue("--dark-main"));
      } else {
        r.style.setProperty("--background", r.style.getPropertyValue("--light-background"));
  ...
  return (
    <>
      <Button
        className="size-9 bg-secondary-background p-0 [&_svg]:size-5"
