  ...
  return (
    <Card className="h-full min-w-0" data-chart-recipe="revenue">
      <CardHeader>
        ...
        <CardTitle role="heading" aria-level={3}>Revenue vs target</CardTitle>
        <CardDescription>Weekly revenue against an explicit plan. Sample data in USD.</CardDescription>
      </CardHeader>
      <CardContent className="grid min-w-0 gap-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <p className="text-xs leading-5 text-foreground/75">Bars: revenue / dashed line: target</p>
        </div>
        ...
        <ChartContainer config={config} className="h-64 w-full min-w-0 aspect-auto" aria-label="Weekly revenue and target">
          <ComposedChart accessibilityLayer data={data} margin={{ top: 12, right: 8, bottom: 0, left: 0 }}>
            ...
            <YAxis tickLine={false} axisLine={false} width={48} tickFormatter={(value) => `$${Number(value) / 1000}k`} domain={[0, "auto"]} />
            <ChartTooltip content={<ChartTooltipContent formatter={(value, name) => `${name === "revenue" ? "Revenue" : "Target"}: ${money.format(Number(value))}`} />} />
            <Bar dataKey="revenue" fill="var(--color-revenue)" maxBarSize={48} radius={[3, 3, 0, 0]} isAnimationActive={false} />
            <Line dataKey="target" type="linear" stroke="var(--color-target)" strokeWidth={2} strokeDasharray="6 4" dot={false} isAnimationActive={false} />
          </ComposedChart>
        </ChartContainer>
        <p className="border-l-4 border-main pl-3 text-sm leading-6" role="status">
          {((revenue / target) * 100).toFixed(1)}% of target achieved. {belowTarget} of {data.length} weeks below plan.
        </p>
        <details className="min-w-0 border-t-2 border-border pt-4">
          ...
          <div className="mt-3 overflow-x-auto" role="region" aria-label="Revenue values" tabIndex={0}>
            <table className="w-full text-left text-xs tabular-nums">
              <caption className="pb-3 text-left text-foreground/75">The same {data.length} weeks shown in the chart. USD.</caption>
              <thead><tr>{["Week", "Revenue", "Target", "Variance"].map((label) => <th key={label} scope="col" className="border-b-2 border-border py-2 pr-3">{label}</th>)}</tr></thead>
              <tbody>{data.map((row) => <tr key={row.week}><th scope="row" className="py-2 pr-3">{row.week}</th><td className="pr-3">{money.format(row.revenue)}</td><td className="pr-3">{money.format(row.target)}</td><td>{money.format(row.revenue - row.target)}</td></tr>)}</tbody>
            </table>
          </div>
        </details>
      </CardContent>
